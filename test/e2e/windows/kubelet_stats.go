	...
	ginkgo.Describe("Kubelet stats collection for Windows nodes", func() {
		ginkgo.Context("when running 10 pods", func() {
			...
			ginkgo.It("should return within 10 seconds", func() {
				...
				ginkgo.By("Scheduling 8 pods")
				powershellImage := imageutils.GetConfig(imageutils.BusyBox)
				pods := newKubeletStatsTestPods(8, powershellImage, targetNode.Name)
				f.PodClient().CreateBatch(pods)

				ginkgo.By("Waiting up to 3 minutes for pods to be running")
				timeout := 3 * time.Minute
				e2epod.WaitForPodsRunningReady(f.ClientSet, f.Namespace.Name, 8, 0, timeout, make(map[string]string))
				...
				for i := 0; i < iterations; i++ {
					...
					framework.ExpectEqual(statsChecked, 8, "Should find stats for 6 pods in kubelet stats")
